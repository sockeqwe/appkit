package com.hannesdorfmann.appkit.string;

import java.util.Random;

/**
 * A static class providing some useful string methods
 * @author Hannes Dorfmann
 */
public class StringUtils {

  /**
   * Determines if a string is empty (is null or empty)
   *
   * @param str
  ...
  /**
   * Checks if two strings are not null and equals each other
   *
   * @param a
   * @param b
   * @return
   */
  public static boolean equalsWithoutNull(String a, String b) {
  ...
  /**
   *
   *  Returns string b if string a is null
   *
   */
   ...
   * </p>
   * <p>
   * Currently <em>private high surrogate</em> characters are ignored. These
   * are Unicode characters that fall between the values 56192 (db80) and
   * 56319 (dbff) as we don't know how to handle them. High and low surrogates
   * are correctly dealt with - that is if a high surrogate is randomly
   * chosen, 55296 (d800) to 56191 (db7f) then it is followed by a low
   * surrogate. If a low surrogate is chosen, 56320 (dc00) to 57343 (dfff)
   * then it is placed after a randomly chosen high surrogate.
   * </p>
   *
   * <p>
   * #ThreadSafe#
  ...
  public static String randomAsciiLettersNumbers(final int count) {
    return random(count, 32, 127, true, true);
  }
   ...
   * internal static {@link java.util.Random} instance.
   * </p>
   *
   * @param count
   *            the length of random string to create
   ...
   *
   * <p>
   * This method accepts a user-supplied {@link java.util.Random} instance to use as a
   * source of randomness. By seeding a single {@link java.util.Random} instance with a
  ...
  public static String replaceHTMLBreak(String toFormat) {
    // <br>|< br>| < br >| <br >| <br /> | < br /> | < br / >| <br / >| < br
    // / >
    return toFormat.replaceAll("\\s*<\\s*(br|BR|Br|bR)\\s*/?\\s*>\\s*",
        "\n");
  }
